...
Это PowerShell-оболочка для *прямого системного монтирования* удалённых хостов через **SSHFS-Win** (на базе WinFsp).

Важно: в Windows этот скрипт **монтирует удалённый хост как отдельный диск (буква X:)**, а не “в папку рядом со скриптом”.
Для удобства он дополнительно создаёт **junction** `.\<host-name>\ -> X:\` рядом со скриптом, но реальная точка монтирования — именно диск.

Из-за этого:
- размонтирование обычно делается как “отключить диск” в Проводнике (или через `-Unmount`),
- в VS Code нужно выбирать **диск (например `X:\`) как папку проекта** (или открыть junction-папку рядом со скриптом, если так удобнее).
